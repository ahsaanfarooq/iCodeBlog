...
@section('main-section')
    ...
    @foreach ($posts as $post)
        ...
        @endphp
    @endforeach
    <title>{{ basename(url()->current()) }} | Category</title>
    <div class="selected_post">
        <div style="font-weight: bolder">{{ basename(url()->current()) }}</div>
        <div>
            @if ($i == 0)
    ...
    <div class="latest_posts">
        ...
        @foreach ($posts as $post)
            ...
            @endphp
            <div class="post_anchor post" style="text-decoration: none">
                ...
                <a style="text-decoration: none" href="{{ url('/post') . '/' . str_replace(' ', '_', $post->title) }}">
                ...
                <div style="display: flex !important;justify-content:space-between; width: 100%;">
                    <a style="text-decoration: none;" class="learn_more"
                        href="{{ url('/post') . '/' . str_replace(' ', '_', $post->title) }}">Read More
                    ...
                    <a href="{{ url('/post') . '/' . str_replace(' ', '_', $post->title) . '#comment' }}"
                        class="comments_count">({{ $commentsNo }}) Comments</a>
                </div>
                <hr class="post_hr">
